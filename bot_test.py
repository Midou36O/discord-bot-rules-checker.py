...
async def on_message(msg): # msg is the message
    if msg.author == bot.user: # Here we make sure that the bot doesn't check his own messages
        ...
        if msg.content.startswith('cactuses'): # Here we look if the message starts with a specific word, like a prefix
          if msg.content[8:].startswith('explodes'): # 8 is the amount of characters in the first word. If you want to add a space between the two words, remember to add the space in the number too
            if msg.content[16:].startswith('balloons'): # for 7, it's the same thing as second word
